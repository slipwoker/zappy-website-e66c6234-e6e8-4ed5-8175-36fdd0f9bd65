  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      if (!emailRegex.test(email)) {
        alert('אנא הזיני כתובת אימייל תקינה');
        return;
      }
      
      alert('תודה על פנייתך! נחזור אליך בהקדם עם פרטי ההצטרפות לקבוצה.');
      contactForm.reset();
    });
  }
});
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "קוסמטולוגיה מתקדמת משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: 'e66c6234-e6e8-4ed5-8175-36fdd0f9bd65',
                    name: data.name || '',
                    email: data.email || '',
